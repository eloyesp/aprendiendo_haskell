...
A heap profiling log, `A.hp`{.code}, was created, with the content in
the following form:
...
The intermediate values in the loop state are now made strict, and the
...
After installing the `uvector`{.code} library, from Hackage, we can
